  ...
  fun createValues(arrowType: ArrowType, n: Int): List<Any?> {
    return when (arrowType) {
      ...
      ArrowTypes.StringType ->
          (0 until n).map { enhancedRandom.nextString(rand.nextInt(64)) }
    ...
    (0 until rowCount).forEach { row ->
      (0 until columns.size).forEach { col ->
        ...
        when (v) {
          ...
          is DateDayVector -> v.set(row, value as Int)
          else -> throw IllegalStateException()
        }
      }
    }
    ...
    return if (depth == maxDepth) {
      ...
      when (rand.nextInt(2)) {
        0 -> {
          ...
          child.project(
              exprCount.map { createExpression(child, 0, maxExprDepth) })
        }
