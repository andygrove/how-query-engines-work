  ...
  private fun pushDown(
    ...
    return when (plan) {
      ...
      is Aggregate -> {
        ...
        extractColumns(
            plan.aggregateExpr.map { it.expr }, plan.input, columnNames)
        val input = pushDown(plan.input, columnNames)
      ...
      is Join -> {
        ...
        Join(left, right, plan.joinType, plan.on)
      }
      is Scan -> {
        val validFieldNames =
            plan.dataSource.schema().fields.map { it.name }.toSet()
        val pushDown =
            validFieldNames.filter { columnNames.contains(it) }.toSet().sorted()
        Scan(plan.path, plan.dataSource, pushDown)
      }
      else ->
          throw IllegalStateException(
              "ProjectionPushDownRule does not support plan: $plan")
    }
  }
}
