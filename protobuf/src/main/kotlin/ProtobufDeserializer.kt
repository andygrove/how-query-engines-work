...
class ProtobufDeserializer {
  ...
  fun fromProto(node: LogicalPlanNode): LogicalPlan {
    return if (node.hasCsvScan()) {
      ...
      val ds =
          CsvDataSource(node.csvScan.path, schema, node.csvScan.hasHeader, 1024)
      Scan(
          node.csvScan.path,
          ds,
          node.csvScan.projection.columnsList.asByteStringList().map {
            it.toString()
          })
      ...
      Projection(
          fromProto(node.input), node.projection.exprList.map { fromProto(it) })
      ...
      val aggrExpr =
          node.aggregate.aggrExprList.map { fromProto(it) as AggregateExpr }
  ...
  fun fromProto(node: LogicalExprNode): LogicalExpr {
    ...
    return when (node.exprTypeCase) {
        ...
        when (binaryNode.op) {
          ...
          "divide" -> Divide(ll, rr)
              ...
              throw RuntimeException(
                  "Failed to parse logical binary expression: $node")
        }
      }
      LogicalExprNode.ExprTypeCase.ALIAS ->
          Alias(fromProto(node.alias.expr), node.alias.alias)
      ...
      LogicalExprNode.ExprTypeCase.LITERAL_INT8 ->
          lit(node.literalInt8.toLong())
      LogicalExprNode.ExprTypeCase.LITERAL_INT16 ->
          lit(node.literalInt16.toLong())
      LogicalExprNode.ExprTypeCase.LITERAL_INT32 ->
          lit(node.literalInt32.toLong())
      LogicalExprNode.ExprTypeCase.LITERAL_INT64 -> lit(node.literalInt64)
      LogicalExprNode.ExprTypeCase.LITERAL_UINT8 ->
          lit(node.literalUint8.toLong())
      LogicalExprNode.ExprTypeCase.LITERAL_UINT16 ->
          lit(node.literalUint16.toLong())
      LogicalExprNode.ExprTypeCase.LITERAL_UINT32 ->
          lit(node.literalUint32.toLong())
      ...
      LogicalExprNode.ExprTypeCase.IS_NULL_EXPR ->
          TODO("Requires that IsNullExpr is implemented in kotlin first")
      LogicalExprNode.ExprTypeCase.IS_NOT_NULL_EXPR ->
  ...
  fun fromProto(schema: Schema): io.andygrove.kquery.datatypes.Schema {
    ...
    val arrowFields =
        schema.columnsList.map {
          ...
          val fieldType =
              org.apache.arrow.vector.types.pojo.FieldType(true, dt, null)
