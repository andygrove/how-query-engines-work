...
class HashAggregateExec(
  ...
  override fun toString(): String {
    return "HashAggregateExec: groupExpr=$groupExpr, aggrExpr=$aggregateExpr, mode=$mode"
  }
  // snippet-omit-end
  ...
  override fun execute(): Sequence<RecordBatch> {
    ...
    map.entries.withIndex().forEach { entry ->
      ...
      aggregateExpr.indices.forEach {
        // In PARTIAL mode, output intermediate values; otherwise output final values
        val outputValue =
            when (mode) {
              AggregateMode.PARTIAL -> accumulators[it].intermediateValue()
              else -> accumulators[it].finalValue()
            }
        builders[groupExpr.size + it].set(rowIndex, outputValue)
      }
    }
