...
import io.andygrove.kquery.datatypes.LiteralValueVector
...
class LiteralStringExpression(val value: String) : Expression {
  override fun evaluate(input: RecordBatch): ColumnVector {
    return LiteralValueVector(
        ArrowTypes.StringType, value.toByteArray(), input.rowCount())
  }
}

/** Represents a literal date value (stored as days since epoch). */
class LiteralDateExpression(val daysSinceEpoch: Int) : Expression {
  override fun evaluate(input: RecordBatch): ColumnVector {
    return LiteralValueVector(
        ArrowTypes.DateDayType, daysSinceEpoch, input.rowCount())
  }
}

/** Represents a literal interval (days) value. */
class LiteralIntervalDaysExpression(val days: Long) : Expression {
  override fun evaluate(input: RecordBatch): ColumnVector {
    return LiteralValueVector(ArrowTypes.Int64Type, days, input.rowCount())
  }
}
