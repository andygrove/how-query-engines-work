...
abstract class BinaryExpression(val l: Expression, val r: Expression) : Expression {
  ...
  private fun coerceTypes(ll: ColumnVector, rr: ColumnVector): Pair<ColumnVector, ColumnVector> {
  ...
  private fun isNumeric(type: ArrowType): Boolean {
    return type is ArrowType.Int || type is ArrowType.FloatingPoint || type is ArrowType.Decimal
  }
...
private class CoercedDoubleVector(private val inner: ColumnVector) : ColumnVector {
  ...
  override fun getValue(i: Int): Any? {
    ...
    return when (value) {
      ...
      else -> throw IllegalStateException("Cannot coerce ${value.javaClass.name} to Double")
    }
  }
