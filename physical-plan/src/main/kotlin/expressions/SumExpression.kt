...
class SumAccumulator : Accumulator {
                      ...
                      when (value) {
                        ...
                        is Number -> BigDecimal.valueOf(value.toDouble())
                            ...
                            throw UnsupportedOperationException(
                                "Cannot add $value to BigDecimal")
                      })
          else ->
              throw UnsupportedOperationException(
                  "SUM is not implemented for type: ${currentValue?.javaClass?.name}")
        }
      }
    }
  }
  ...
  override fun merge(other: Any?) {
    // For SUM, merge is the same as accumulate - just add the partial sum
    accumulate(other)
  }
}
