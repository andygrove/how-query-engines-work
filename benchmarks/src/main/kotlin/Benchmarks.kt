...
/**
 * Designed to be run from Docker. See top-level benchmarks folder for more
 * info.
 */
class Benchmarks {
  companion object {
...
private fun sqlAggregate(
  ...
  val deferred =
      files.map { file ->
        GlobalScope.async {
          ...
          val result =
              executeQuery(
                  File(File(path), file).absolutePath, sqlPartial, settings)
  ...
  ctx.registerDataSource(
      "tripdata", InMemoryDataSource(results.first().schema, results))
