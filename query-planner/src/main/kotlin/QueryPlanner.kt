...
class QueryPlanner {
  ...
  fun createPhysicalPlan(plan: LogicalPlan): PhysicalPlan {
    return when (plan) {
      ...
      is Projection -> {
        val input = createPhysicalPlan(plan.input)
        val projectionExpr =
            plan.expr.map { createPhysicalExpr(it, plan.input) }
      ...
      is Aggregate -> {
        val input = createPhysicalPlan(plan.input)
        val groupExpr =
            plan.groupExpr.map { createPhysicalExpr(it, plan.input) }
        val aggregateExpr =
            plan.aggregateExpr.map {
              when (it) {
                ...
                is Count ->
                    CountExpression(createPhysicalExpr(it.expr, plan.input))
                else ->
            ...
            plan.on.map { (leftCol, _) ->
              leftSchema.fields
                  ...
                  .also {
                    if (it == -1)
                        throw SQLException(
                            "No column named '$leftCol' in left input")
                  }
            }
        val rightKeys =
            plan.on.map { (_, rightCol) ->
              rightSchema.fields
                  ...
                  .also {
                    if (it == -1)
                        throw SQLException(
                            "No column named '$rightCol' in right input")
                  }
            }

        // Find right columns to exclude (duplicate key columns with same name)
        val duplicateKeyNames =
            plan.on.filter { it.first == it.second }.map { it.second }.toSet()
  ...
  fun createPhysicalExpr(expr: LogicalExpr, input: LogicalPlan): Expression =
      when (expr) {
        ...
        is LiteralDate -> LiteralDateExpression(expr.value.toEpochDay().toInt())
        is LiteralIntervalDays -> LiteralIntervalDaysExpression(expr.days)
        is DateSubtractInterval ->
            DateSubtractIntervalExpression(
                createPhysicalExpr(expr.dateExpr, input),
                createPhysicalExpr(expr.intervalExpr, input))
        is DateAddInterval ->
        ...
        is CastExpr ->
            CastExpression(createPhysicalExpr(expr.expr, input), expr.dataType)
        is BinaryExpr -> {
          ...
          when (expr) {
            ...
            throw IllegalStateException("Unsupported logical expression: $expr")
      }
}
