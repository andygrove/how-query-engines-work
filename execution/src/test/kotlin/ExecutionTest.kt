...
class ExecutionTest {
  ...
  fun `date and interval arithmetic`() {
    ...
    val input =
        Fuzzer()
            .createRecordBatch(schema, listOf(listOf(1, 2, 3, 4), listOf(sep1, oct1, nov1, dec1)))
    ...
    val df =
        ctx.sql("SELECT id FROM orders WHERE ship_date <= date '1998-12-01' - interval '68 days'")
