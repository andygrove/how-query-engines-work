...
class SqlTokenizer(val sql: String) {
  ...
  private fun nextToken(): Token? {
    ...
    when {
      ...
      Literal.isCharsStart(sql[offset]) -> {
        ...
        throw TokenizeException("Unexpected character '${sql[offset]}' at position $offset")
      }
    }
  }
  ...
  private fun scanIdentifier(startOffset: Int): Token {
    if ('`' == sql[startOffset]) {
      ...
      return Token(sql.substring(startOffset + 1, endOffset), Literal.IDENTIFIER, endOffset + 1)
    }
  ...
  private fun getOffsetUntilTerminatedChar(terminatedChar: Char, startOffset: Int): Int {
    ...
    else throw TokenizeException("Must contain $terminatedChar in remain sql[$startOffset .. end]")
  }
  ...
  private fun scanChars(startOffset: Int, terminatedChar: Char): Token {
    ...
    throw TokenizeException("Unterminated string starting at position $startOffset")
  }
