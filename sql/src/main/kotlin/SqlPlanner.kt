...
class SqlPlanner {
  ...
  private fun planAggregateQuery(
    ...
    var plan = df
    val projectionWithoutAggregates = projectionExpr.filterNot { isAggregateExpr(it) }
    ...
    if (select.selection != null) {
      ...
      val missing = (allRequiredColumns - columnNamesInProjectionWithoutAggregates)
      logger.info("** missing: $missing")
          ...
          when (expr.id.uppercase()) {
            ...
            "AVG" -> {
              if (expr.args.isEmpty()) {
                throw SQLException("${expr.id.uppercase()}() requires an argument")
              }
            ...
            "COUNT" -> {
              if (expr.args.isEmpty()) {
                throw SQLException("COUNT() requires an argument, use COUNT(*) to count all rows")
              }
  ...
  private fun parseInterval(value: String): LiteralIntervalDays {
    ...
    val match =
        regex.matchEntire(value.trim())
            ?: throw SQLException("Invalid interval format: '$value'. Expected format: 'N days'")
