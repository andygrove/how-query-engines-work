...
class SqlParser(val tokens: TokenStream) : PrattParser {
  ...
  override fun parsePrefix(): SqlExpr? {
    ...
    val expr =
        when (token.type) {
          ...
          Keyword.DATE -> parseDate()
          Keyword.INTERVAL -> parseInterval()
          Keyword.MIN,
          Keyword.MAX,
          Keyword.SUM,
          Keyword.AVG,
          Keyword.COUNT -> SqlIdentifier(token.text)
          ...
          Symbol.LEFT_PAREN -> {
            val expr =
                parseExpr()
                    ?: throw SQLException("Expected expression after '('")
            if (!tokens.consumeTokenType(Symbol.RIGHT_PAREN)) {
              throw SQLException("Expected ')' after expression")
            }
            expr
          }

          // Star for count(*)
          Symbol.STAR -> SqlIdentifier("*")
        ...
        when (token.type) {
          ...
          Symbol.LEFT_PAREN -> {
            if (left is SqlIdentifier) {
              tokens.next() // consume the left paren
              // Check for empty argument list
              val args =
                  if (tokens.peek()?.type == Symbol.RIGHT_PAREN) {
                    listOf()
                  } else {
                    parseExprList()
                  }
              if (tokens.next()?.type != Symbol.RIGHT_PAREN) {
                throw SQLException("Expected ')' after function arguments")
              }
  ...
  private fun parseCast(): SqlCast {
    if (!tokens.consumeTokenType(Symbol.LEFT_PAREN)) {
      throw SQLException("Expected '(' after CAST")
    }
    val expr = parseExpr() ?: throw SQLException("Expected expression in CAST")
    if (expr !is SqlAlias) {
      throw SQLException("Expected 'AS type' in CAST expression")
    }
    if (!tokens.consumeTokenType(Symbol.RIGHT_PAREN)) {
      throw SQLException("Expected ')' after CAST expression")
    }
    return SqlCast(expr.expr, expr.alias)
  }

  private fun parseDate(): SqlDate {
    val token =
        tokens.next()
            ?: throw SQLException("Expected date string after DATE keyword")
    if (token.type != Literal.STRING) {
      throw SQLException(
          "Expected date string after DATE keyword, found $token")
    }
    return SqlDate(token.text)
  }

  private fun parseInterval(): SqlInterval {
    val token =
        tokens.next()
            ?: throw SQLException(
                "Expected interval string after INTERVAL keyword")
    if (token.type != Literal.STRING) {
      throw SQLException(
          "Expected interval string after INTERVAL keyword, found $token")
    }
    return SqlInterval(token.text)
  }
  ...
  private fun parseSelect(): SqlSelect {
    ...
    if (tokens.consumeKeyword("FROM")) {
      val tableExpr =
          parseExpr() ?: throw SQLException("Expected table name after FROM")
      if (tableExpr !is SqlIdentifier) {
        throw SQLException("Expected table name after FROM, found $tableExpr")
      }
      val table = tableExpr
      ...
      if (tokens.consumeKeyword("LIMIT")) {
        val limitExpr =
            parseExpr()
                ?: throw SQLException("Expected limit value after LIMIT")
        limit =
            when (limitExpr) {
      ...
      return SqlSelect(
          projection, filterExpr, groupBy, orderBy, havingExpr, limit, table.id)
    } else {
      throw IllegalStateException(
          "Expected FROM keyword, found ${tokens.peek()}")
    }
  }
  ...
  /**
   * Parse the next token as an identifier, throwing an exception if the next
   * token is not an identifier.
   */
  private fun parseIdentifier(): SqlIdentifier {
    val expr =
        parseExpr() ?: throw SQLException("Expected identifier, found EOF")
    return when (expr) {
