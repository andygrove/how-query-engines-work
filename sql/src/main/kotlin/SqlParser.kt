...
class SqlParser(val tokens: TokenStream) : PrattParser {
  ...
  override fun parsePrefix(): SqlExpr? {
    ...
    val expr =
        when (token.type) {
          ...
          Symbol.LEFT_PAREN -> {
            val expr = parseExpr() ?: throw SQLException("Expected expression after '('")
  ...
  private fun parseDate(): SqlDate {
    val token = tokens.next() ?: throw SQLException("Expected date string after DATE keyword")
    if (token.type != Literal.STRING) {
      throw SQLException("Expected date string after DATE keyword, found $token")
    }
  ...
  private fun parseInterval(): SqlInterval {
    val token =
        tokens.next() ?: throw SQLException("Expected interval string after INTERVAL keyword")
    if (token.type != Literal.STRING) {
      throw SQLException("Expected interval string after INTERVAL keyword, found $token")
    }
  ...
  private fun parseSelect(): SqlSelect {
    ...
    if (tokens.consumeKeyword("FROM")) {
      val tableExpr = parseExpr() ?: throw SQLException("Expected table name after FROM")
