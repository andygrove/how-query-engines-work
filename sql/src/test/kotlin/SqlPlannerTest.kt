...
class SqlPlannerTest {
  ...
  fun `COUNT without argument should error`() {
    val exception = assertThrows<SQLException> { plan("SELECT COUNT() FROM employee") }
    assertEquals("COUNT() requires an argument, use COUNT(*) to count all rows", exception.message)
  }

  @Test
  fun `MAX without argument should error`() {
    val exception = assertThrows<SQLException> { plan("SELECT MAX() FROM employee") }
    assertEquals("MAX() requires an argument", exception.message)
  }

  @Test
  fun `MIN without argument should error`() {
    val exception = assertThrows<SQLException> { plan("SELECT MIN() FROM employee") }
    assertEquals("MIN() requires an argument", exception.message)
  }

  @Test
  fun `SUM without argument should error`() {
    val exception = assertThrows<SQLException> { plan("SELECT SUM() FROM employee") }
    assertEquals("SUM() requires an argument", exception.message)
  }

  @Test
  fun `AVG without argument should error`() {
    val exception = assertThrows<SQLException> { plan("SELECT AVG() FROM employee") }
