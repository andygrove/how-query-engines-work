...
class SqlTokenizerTest {
  ...
  fun `tokenize SELECT with aggregates`() {
    ...
    val actual =
        SqlTokenizer("SELECT state, MAX(salary) FROM employee GROUP BY state").tokenize().tokens
    assertEquals(expected, actual)
  }
  ...
  fun `tokenize date literal`() {
    val expected = listOf(Token("date", Keyword.DATE, 4), Token("1998-12-01", Literal.STRING, 17))
  ...
  fun `tokenize interval literal`() {
    val expected =
        listOf(Token("interval", Keyword.INTERVAL, 8), Token("68 days", Literal.STRING, 18))
