...
class SqlParserTest {
  ...
  fun `parse SELECT with date literal in WHERE`() {
    val select = parseSelect("SELECT id FROM orders WHERE order_date < date '1998-12-01'")
    assertEquals(listOf(SqlIdentifier("id")), select.projection)
    assertEquals(
        SqlBinaryExpr(SqlIdentifier("order_date"), "<", SqlDate("1998-12-01")), select.selection)
  ...
  fun `function call missing closing paren should error`() {
    val exception = assertThrows<SQLException> { parse("SELECT MAX(salary FROM employee") }
  ...
  fun `CAST missing opening paren should error`() {
    val exception =
        assertThrows<SQLException> { parse("SELECT CAST salary AS double) FROM employee") }
    assertEquals("Expected '(' after CAST", exception.message)
  }

  @Test
  fun `CAST missing AS should error`() {
    val exception = assertThrows<SQLException> { parse("SELECT CAST(salary) FROM employee") }
  ...
  fun `FROM with string literal should error`() {
    val exception = assertThrows<SQLException> { parse("SELECT a FROM 'table'") }
    assert(exception.message!!.contains("Expected table name after FROM"))
  }
}
