...
impl AggregateExpr for Max {
    ...
    fn data_type(&self, input_schema: &Schema) -> Result<DataType> {
        match self.expr.data_type(input_schema)? {
            ...
            other => Err(ballista_error(&format!("MAX does not support {:?}", other))),
        }
    }
        ...
        match value {
            ColumnarValue::Columnar(array) => {
                let max = match array.data_type() {
                    ...
                    _ => Err(ballista_error("Unsupported data type for MAX")),
                }?;
                if let Some(max) = max {
                    self.accumulate(&ColumnarValue::Scalar(max, 1))
                } else {
                    Ok(())
                }
            }
            ColumnarValue::Scalar(value, _n) => {
                match value {
                    ...
                    other => {
                        return Err(ballista_error(&format!("MAX does not support {:?}", other)))
                    }
                }
                Ok(())
            }
...
pub fn max(expr: Arc<dyn Expression>) -> Arc<dyn AggregateExpr> {
