...
class CsvDataSourceTest {
  ...
  fun `read csv with no projection`() {
    val csv =
        CsvDataSource(File(dir, "employee.csv").absolutePath, null, true, 1024)
  ...
  fun `read csv with projection`() {
    ...
    val headers =
        listOf("first_name", "last_name", "state", "job_title", "salary")
  ...
  fun `read csv with small batch`() {
    val csv =
        CsvDataSource(File(dir, "employee.csv").absolutePath, null, true, 1)
  ...
  fun `read csv with no header`() {
    val csv =
        CsvDataSource(
            File(dir, "employee_no_header.csv").absolutePath, null, false, 1024)
    val result = csv.scan(listOf())
    val headers =
        listOf("field_1", "field_2", "field_3", "field_4", "field_5", "field_6")
    result.asSequence().forEach {
      val field = it.field(0)
      assert(field.size() == 4)
    }
  ...
  fun `read tsv with no projection`() {
    val csv =
        CsvDataSource(File(dir, "employee.tsv").absolutePath, null, true, 1024)
  ...
  fun `read tsv with small batch`() {
    val csv =
        CsvDataSource(File(dir, "employee.tsv").absolutePath, null, true, 1)
    val result = csv.scan(listOf()).asSequence().toList()
  ...
  fun `read tsv with no header`() {
    val csv =
        CsvDataSource(
            File(dir, "employee_no_header.tsv").absolutePath, null, false, 1024)
    val result = csv.scan(listOf())
    val headers =
        listOf("field_1", "field_2", "field_3", "field_4", "field_5", "field_6")
    result.asSequence().forEach {
