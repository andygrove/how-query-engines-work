...
class ParquetDataSourceTest {
  ...
  fun `read parquet schema`() {
    val parquet =
        ParquetDataSource(File(dir, "alltypes_plain.parquet").absolutePath)
    assertEquals(
  ...
  @Test
  fun `read parquet string column`() {
    val parquet =
        ParquetDataSource(File(dir, "alltypes_plain.parquet").absolutePath)
    val it = parquet.scan(listOf("string_col")).iterator()
    assertTrue(it.hasNext())
    ...
    val stringCol = batch.field(0)
    // Verify values are not null
    for (i in 0 until stringCol.size()) {
      assertNotNull(
          stringCol.getValue(i), "String value at index $i should not be null")
    }
