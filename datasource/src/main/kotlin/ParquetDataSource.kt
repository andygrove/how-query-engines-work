        ...
        if (group.getFieldRepetitionCount(fieldName) == 1) {
          when (fieldType.asPrimitiveType().primitiveTypeName) {
            PrimitiveType.PrimitiveTypeName.BOOLEAN -> {
              (vector as BitVector).set(rowIndex, if (group.getBoolean(fieldName, 0)) 1 else 0)
            }
            PrimitiveType.PrimitiveTypeName.INT32 -> {
              (vector as IntVector).set(rowIndex, group.getInteger(fieldName, 0))
            }
            PrimitiveType.PrimitiveTypeName.INT64 -> {
              ...
              when (vector) {
                ...
                is DecimalVector -> {
                  ...
                  val unscaledValue = BigDecimal.valueOf(longValue, vector.scale)
            ...
            PrimitiveType.PrimitiveTypeName.FLOAT -> {
              (vector as Float4Vector).set(rowIndex, group.getFloat(fieldName, 0))
            }
            PrimitiveType.PrimitiveTypeName.DOUBLE -> {
              (vector as Float8Vector).set(rowIndex, group.getDouble(fieldName, 0))
            }
